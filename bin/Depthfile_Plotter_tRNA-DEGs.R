...
#setwd("/home/paul/Documents/Pipelines/work/e2/cae4d1a86791f33be378efa5cdb0f3/")
...
genelist <- read.table(args[3], header = T) # File with list of genes to plot
#input1 <- read.table("Everything_CytC_concatenated_mean_stdev_tsRNA.depth")
#input2 <- read.table("Everything_TotalRNA_concatenated_mean_stdev_tsRNA.depth")
#genelist <- read.table("High-combined-score-features_feature-names.txt", header = T) # File with list of genes to plot
genelist$V1 <- factor(genelist[,1], levels=unique(genelist[,1])) # Reassign the factor to stop reordering of factors when converting DF column to list
...
if (file.size(args[3]) == 0) { # Check if features file contains any lines
  ...
  #GTF <- read.table("mouse_ncRNAs_relative_cdhit.gtf", sep = "\t")
  
  ### Get names of conditions from arg3 name for writing in plots
  file3 <- basename(args[4])
  #file3 <- "CytC_vs_TotalRNA_Features_Top-ncRNAs.pdf"
  ...
  if(flag == "no") {   
    ...
    for(feature in genelist){
      print(paste0("Plotting transcriptional profile for ", feature))
    ...
    for(feature in featuresUnion) {
      ...
      ### file 1
      feature.full <- paste0("^", feature, "$", sep="")
      input1[grep(feature.full, input1$V1),] # \\ + \\b denote 'boundary' of word we are searching for. Stops partial matches. Equivalent to ^feature$
